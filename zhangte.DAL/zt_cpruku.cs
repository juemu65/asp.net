...
namespace zhangte.DAL
{
	...
	public partial class zt_cpruku
	{
		...
		/// 增加一条数据
		/// </summary>
        public int Add(zhangte.Model.zt_cpruku model)
		{
			StringBuilder strSql=new StringBuilder();
         
			strSql.Append("insert into zt_cpruku(");
			strSql.Append("企业标识代码,产品名称,数量,入库原因,入库时间)  values (");
			strSql.Append("@企业标识代码,@产品名称,@数量,@入库原因,@入库时间)");
			strSql.Append(";");
			MySqlParameter[] parameters = {
					...
					new MySqlParameter("@入库原因", MySqlDbType.VarChar,255),
					new MySqlParameter("@入库时间", MySqlDbType.VarChar,255)};
			parameters[0].Value = model.企业标识代码;
			parameters[1].Value = model.产品名称;
			parameters[2].Value = model.数量;
			parameters[3].Value = model.入库原因;
			parameters[4].Value = model.入库时间;
		...
		/// 更新一条数据
		/// </summary>
        public bool Update(zhangte.Model.zt_cpruku model)
		{
			StringBuilder strSql=new StringBuilder();

            strSql.Append("update zt_cpruku set ");
            strSql.Append("企业标识代码=@企业标识代码,");
            strSql.Append("产品名称=@产品名称,");
            strSql.Append("数量=@数量,");
            strSql.Append("入库原因=@入库原因,");
		...
		public bool Delete(int Id)
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
            strSql.Append("delete from zt_cpruku ");
            strSql.Append(" where Id=@Id");
            MySqlParameter[] parameters = {
		...
		/// 得到一个对象实体
		/// </summary>
        public zhangte.Model.zt_cpruku GetModel(int Id)
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select   * from zt_cpruku ");
            ...
            zhangte.Model.zt_cpruku model = new zhangte.Model.zt_cpruku();
		...
		/// 得到一个对象实体
		/// </summary>
		public zhangte.Model.zt_cpruku DataRowToModel(DataRow row)
		{
			zhangte.Model.zt_cpruku model=new zhangte.Model.zt_cpruku();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM zt_cpruku ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
        ...
        private DataSet GetListByPage(string columslist, string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from zt_cpruku T ");
        ...
        {
            string str = DbHelperMySQL.GetListByPageColumns_tojson(columslist, strWhere, orderby, PageSize, PageIndex, "zt_cpruku");
        ...
        {
            string str = DbHelperMySQL.GetListByColumns_tojson(columslist, strWhere, orderby, "zt_cpruku");
        ...
        {
            string strsql = DbHelperMySQL.GetListByPageColumns(columslist, strWhere, orderby, PageSize, PageIndex, "zt_cpruku");
