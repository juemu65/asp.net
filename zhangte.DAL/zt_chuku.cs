...
namespace zhangte.DAL
{
	...
	public partial class zt_chuku
	{
		...
		/// 增加一条数据
		/// </summary>
        public int Add(zhangte.Model.zt_chuku model)
		{
			StringBuilder strSql=new StringBuilder();
         
			strSql.Append("insert into zt_chuku(");
			strSql.Append("企业标识代码,产品名称,数量,出库原因,出库时间)  values (");
			strSql.Append("@企业标识代码,@产品名称,@数量,@出库原因,@出库时间)");
			strSql.Append(";");
			MySqlParameter[] parameters = {
					...
					new MySqlParameter("@出库原因", MySqlDbType.VarChar,255),
					new MySqlParameter("@出库时间", MySqlDbType.VarChar,255)};
			parameters[0].Value = model.企业标识代码;
			parameters[1].Value = model.产品名称;
			parameters[2].Value = model.数量;
			parameters[3].Value = model.出库原因;
			parameters[4].Value = model.出库时间;
		...
		/// 更新一条数据
		/// </summary>
        public bool Update(zhangte.Model.zt_chuku model)
		{
			StringBuilder strSql=new StringBuilder();

            strSql.Append("update zt_chuku set ");
            strSql.Append("企业标识代码=@企业标识代码,");
            strSql.Append("产品名称=@产品名称,");
            strSql.Append("数量=@数量,");
            strSql.Append("出库原因=@出库原因,");
            strSql.Append("出库时间=@出库时间");
		...
		public bool Delete(int Id)
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
            strSql.Append("delete from zt_chuku ");
            strSql.Append(" where Id=@Id");
            MySqlParameter[] parameters = {
		...
		/// 得到一个对象实体
		/// </summary>
        public zhangte.Model.zt_chuku GetModel(int Id)
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select   * from zt_chuku ");
            ...
            zhangte.Model.zt_chuku model = new zhangte.Model.zt_chuku();
		...
		/// 得到一个对象实体
		/// </summary>
		public zhangte.Model.zt_chuku DataRowToModel(DataRow row)
		{
			zhangte.Model.zt_chuku model=new zhangte.Model.zt_chuku();
			if (row != null)
			{
                ...
                if (row["出库原因"] != null )
                {
                    model.出库原因 = row["出库原因"].ToString();
                }
                if (row["出库时间"] != null )
                {
                    model.出库时间 = row["出库时间"].ToString();
                }
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM zt_chuku ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
        ...
        private DataSet GetListByPage(string columslist, string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from zt_chuku T ");
        ...
        {
            string str = DbHelperMySQL.GetListByPageColumns_tojson(columslist, strWhere, orderby, PageSize, PageIndex, "zt_chuku");
        ...
        {
            string str = DbHelperMySQL.GetListByColumns_tojson(columslist, strWhere, orderby, "zt_chuku");
        ...
        {
            string strsql = DbHelperMySQL.GetListByPageColumns(columslist, strWhere, orderby, PageSize, PageIndex, "zt_chuku");
