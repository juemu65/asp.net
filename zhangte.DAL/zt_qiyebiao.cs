...
namespace zhangte.DAL
{
	...
	public partial class zt_qiyebiao
	{
		...
		/// 增加一条数据
		/// </summary>
        public int Add(zhangte.Model.zt_qiyebiao model)
		{
			StringBuilder strSql=new StringBuilder();
         
			strSql.Append("insert into zt_qiyebiao(");
			strSql.Append("企业标识代码,登录密码,企业名称,企业类别,联系方式,发货地址,信用等级)  values (");
			strSql.Append("@企业标识代码,@登录密码,@企业名称,@企业类别,@联系方式,@发货地址,@信用等级)");
			strSql.Append(";");
			MySqlParameter[] parameters = {
					new MySqlParameter("@企业标识代码", MySqlDbType.VarChar,255),
					new MySqlParameter("@登录密码", MySqlDbType.VarChar,255),
					new MySqlParameter("@企业名称", MySqlDbType.VarChar,255),
					new MySqlParameter("@企业类别", MySqlDbType.VarChar,255),
					new MySqlParameter("@联系方式", MySqlDbType.VarChar,255),
					new MySqlParameter("@发货地址", MySqlDbType.VarChar,255),
					new MySqlParameter("@信用等级", MySqlDbType.VarChar,255)};
			parameters[0].Value = model.企业标识代码;
			parameters[1].Value = model.登录密码;
			parameters[2].Value = model.企业名称;
			parameters[3].Value = model.企业类别;
			parameters[4].Value = model.联系方式;
			parameters[5].Value = model.发货地址;
			parameters[6].Value = model.信用等级;
		...
		/// 更新一条数据
		/// </summary>
        public bool Update(zhangte.Model.zt_qiyebiao model)
		{
			StringBuilder strSql=new StringBuilder();

            strSql.Append("update zt_qiyebiao set ");
            strSql.Append("企业标识代码=@企业标识代码,");
            strSql.Append("登录密码=@登录密码,");
            strSql.Append("企业名称=@企业名称,");
            strSql.Append("企业类别=@企业类别,");
            strSql.Append("联系方式=@联系方式,");
            strSql.Append("发货地址=@发货地址,");
            strSql.Append("信用等级=@信用等级");
		...
		public bool Delete(int Id)
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
            strSql.Append("delete from zt_qiyebiao ");
            strSql.Append(" where Id=@Id");
            MySqlParameter[] parameters = {
        ...
        public zhangte.Model.zt_qiyebiao GetModel(int Id)
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select   * from zt_qiyebiao ");
		...
		/// 得到一个对象实体
		/// </summary>
		public zhangte.Model.zt_qiyebiao DataRowToModel(DataRow row)
		{
			zhangte.Model.zt_qiyebiao model=new zhangte.Model.zt_qiyebiao();
			if (row != null)
			{
                ...
                if (row["登录密码"] != null )
                {
                    model.登录密码 = row["登录密码"].ToString();
                }
                if (row["企业名称"] != null )
                {
                    model.企业名称 = row["企业名称"].ToString();
                }
                if (row["企业类别"] != null )
                {
                    model.企业类别 = row["企业类别"].ToString();
                }
                if (row["联系方式"] != null )
                {
                ...
                if (row["发货地址"] != null )
                {
                    model.发货地址 = row["发货地址"].ToString();
                }
                if (row["信用等级"] != null )
                {
                    model.信用等级 = row["信用等级"].ToString();
                }
        ...
        public DataSet GetList(string strWhere)
        {
            ...
            strSql.Append(" FROM zt_qiyebiao ");
            if (strWhere.Trim() != "")
            {
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM zt_qiyebiao ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
        ...
        private DataSet GetListByPage(string columslist, string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from zt_qiyebiao T ");
        ...
        {
            string str = DbHelperMySQL.GetListByPageColumns_tojson(columslist, strWhere, orderby, PageSize, PageIndex, "zt_qiyebiao");
        ...
        {
            string str = DbHelperMySQL.GetListByColumns_tojson(columslist, strWhere, orderby, "zt_qiyebiao");
        ...
        {
            string strsql = DbHelperMySQL.GetListByPageColumns(columslist, strWhere, orderby, PageSize, PageIndex, "zt_qiyebiao");
