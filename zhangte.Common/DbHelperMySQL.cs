...
namespace zhangte.Common
{
    /// <summary>
    /// 数据访问抽象基础类
    /// Copyright (C) 宁波大学科学技术学院
    /// </summary>
    public class DbHelperMySQL
    {
        ...
        public static int ExecuteSqlTran(List<CommandInfo> list, List<CommandInfo> oracleCmdSqlList)
        {
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    //bool res =  ExecuteSqlTran(oraConnectionString, oracleCmdSqlList);
                    //if (!res)
                    //{
                    //    tx.Rollback();
        ...
        public static string GetListByPageColumns(string columslist, string strWhere, string orderby, int PageSize, int PageIndex, string datatablename)
        {
            ...
            if (topnum <= 0)
                strsql = "select top " + PageSize + " " + columslist + " from " + datatablename + " " + strwhere1 + orderby;
            else
                strsql = "select top " + PageSize + " " + columslist + " from  " + datatablename + "  where id not in (select top " + topnum + " id from  " + datatablename + "  " + strwhere1 + orderby + ") " + strWhere + orderby;
            return strsql;
        ...
        public static string GetListByPageColumns_tojson(string columslist, string strWhere, string orderby, int PageSize, int PageIndex, string datatablename)
        {
            ...
            if (topnum <= 0)
                ...
                strsql = "select  " + columslist + " from  " + datatablename + "  where id >= (select   id from  " + datatablename + "  " + strwhere1 + orderby + " limit " + topnum + ",1) " + strWhere + orderby;
            //return strsql;
            DataSet ds = new DataSet();
            ds = Query(strsql);
            string allcount = getvalue("select count(*) from  " + datatablename);
            if (allcount == "")
                allcount = "0";
        ...
        public static string GetListByColumns_tojson(string columslist, string strWhere, string orderby, string datatablename)
        {
            if (orderby.Trim() == "")
                orderby = " order by id desc";
            else
        ...
        /// 分页获取数据列表
        /// </summary>
        public static DataSet GetList(int PageSize, int PageIndex, string strWhere, string fieldslist, string tablename, string ordername, string OrderType)
        {
            MySqlParameter[] parameters = {
					new MySqlParameter("@tblName", MySqlDbType.VarChar, 255),
					new MySqlParameter("@fldName", MySqlDbType.VarChar, 255),
					new MySqlParameter("@PageSize", MySqlDbType.Int32,11),
					new MySqlParameter("@PageIndex", MySqlDbType.Int32,11),
					new MySqlParameter("@Ordername", MySqlDbType.Bit),
					new MySqlParameter("@OrderType", MySqlDbType.Bit),
					new MySqlParameter("@strWhere", MySqlDbType.VarChar,1000),
					};
            parameters[0].Value = tablename;
            parameters[1].Value = fieldslist;
