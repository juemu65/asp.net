...
namespace zhangte.ashx
{
    ...
    /// 编辑说明：
    /// </summary>
    public class zcpruku : baseashxClass
    {
        
        public override void ProcessRequest(HttpContext context)
        {
            zhangte.BLL.zt_cpruku bll = new zhangte.BLL.zt_cpruku(context);
            zhangte.Model.zt_cpruku model = new Model.zt_cpruku();
            ...
            else if (context.Request.QueryString["type"] == "save")//保存修改或添加 
            {
                    ...
                    string rukuyuanyin = context.Request.QueryString["rukuyuanyin"];
                    string rukushijian = context.Request.QueryString["rukushijian"];

                
                if (context.Request.QueryString["Id"] != null && context.Request.QueryString["Id"] != "")
                {
                    bll.Update(context.Request.QueryString["Id"].ToString(),qiyebiaoshiDM,chanpinmingcheng,shuliang,rukuyuanyin,rukushijian);
                }
                else
                {
                    bll.Add(qiyebiaoshiDM,chanpinmingcheng,shuliang,rukuyuanyin,rukushijian);
            ...
            else if (Convert.ToString(context.Request.Form["action"]) == "query")
            {
                ...
                string strret = bll.GetListByPageColumns_tojson("Id,企业标识代码,产品名称,数量,入库原因,入库时间", pagestrif, "Id");
                context.Response.Write(strret);
                return;
            }
            else
